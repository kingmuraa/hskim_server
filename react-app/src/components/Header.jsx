...
const Header = () => {
    ...
    return (
        <section className="sticky top-0 left-0 right-0 z-50">
            <header
                className="container mx-auto px-5 flex justify-between py-4 items-center">
                <div>
                ...
                <div
                    ...
                    } transition-all duration-300 mt-[70px] lg:mt-0 bg-dark-hard lg:bg-transparent z-[49] flex flex-col w-full lg:w-auto justify-center lg:justify-end lg:flex-row fixed top-0 bottom-0 lg:static items-center`}>
                    <ul className="text-white items-center gap-y-5 lg:text-dark-hard flex flex-col lg:flex-row gap-x-2 font-semibold">
                        {navItemsInfo.map((item) => (
                        <NavItem key={item.name} name={item.name} item={item}/>
                        ))}
                    </ul>
                    <button
                        className="lg:ml-10 mt-5 lg:mt-0 rounded-full border-2 border-blue-500 px-6 py-2 round-full text-blue-500 font-semibold hover:bg-blue-500 hover:text-white transition-all duration-300">
                        Sign in
                    </button>
                </div>
            </header>
